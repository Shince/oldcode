...
public class DeleteUploadManage extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public DeleteUploadManage() {
		super();
	}
	...
	throws ServletException, IOException {
		...
		if(fileuid==null){
			...
			out.println("<head></head><body onload=\"javascript:parent.location='/CDUG/jsp/AdminManagement/db2resource.jsp';alert('至少选择一个文件.');\"></body>");
		}
		else{
					...
					if (rs.next()) {
						...
						if(flag){
							try{ 
								...
								PrintWriter out = response.getWriter();
								out.println("<head></head><body onload=\"javascript:parent.location='/CDUG/jsp/AdminManagement/db2resource.jsp';alert('删除失败!');\"></body>");
							} 
							++n;
						}
						else{
		...
		PrintWriter out = response.getWriter();
		out.println("<head></head><body onload=\"javascript:parent.location='/CDUG/jsp/AdminManagement/db2resource.jsp';alert('成功删除"+n+"个文件!');\"></body>");
		}
		}
