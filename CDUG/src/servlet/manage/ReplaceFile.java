...
public class ReplaceFile extends HttpServlet {
					...
					if (ServletFileUpload.isMultipartContent(request)) {
						...
						while (fii.hasNext()) {
							...
							if (!fis.isFormField()
								...
								DAO dbctr = new DAO();
								String strSql1 = "select * from cdug_upload_file where filename = '"
										+ fileName_db
										+ "' and date = '"
										+ old_date
										+ "';";
								int row = dbctr.getTableRowCount(
										"cdug_upload_file", strSql1);
								if (row == 0) {
									response
											.getWriter()
											.println(
													"<head></head><body onload=\"javascript:parent.location='"+path+"';alert('未找到该记录!');\"></body>");
								} else {
									BufferedInputStream in = new BufferedInputStream(
											fis.openStream());// 获得文件输入流
									BufferedOutputStream out = new BufferedOutputStream(
											new FileOutputStream(
													new File("C:\\Apache Software Foundation\\Tomcat 5.5\\webapps\\CDUG\\source\\Upload\\"+ old_date + "\\"
															+ fileName)));// 获得文件输出流
						...
						response
								.getWriter()
								.println(
										"<head></head><body onload=\"javascript:parent.location='"+path+"';alert('替换成功');\"></body>");
					}
						...
						response
								.getWriter()
								.println(
										"<head></head><body onload=\"javascript:parent.location='"+path+"';alert('内部错误');\"></body>");
					}
				} catch (Exception e) {
					e.printStackTrace();
					response
							.getWriter()
							.println(
									"<head></head><body onload=\"javascript:parent.location='"+path+"';alert('附件未找到!');\"></body>");
				}
			} else {
				response
						.getWriter()
						.println(
								"<head></head><body onload=\"javascript:parent.location='"+path+"';alert('空文件名！');\"></body>");
			}
		}
	}
