...
public class News extends HttpServlet {
	...
	throws ServletException, IOException {
			...
			if(!(useraccess.equals("1")||useraccess.equals("2"))){
				...
				String strSql2 = "insert into cdug_release_all (title,text,userip,date,tag,username,title_text,text_text) values ('"+title+"','"+text+"','"+userip+"','"+date+"','news','"+username+"','"+title_before+"','"+text_before+"');";
				boolean flag = false; 
				DAO dbctr = new DAO();
				flag = dbctr.ResultUpdate(strSql2);
				if(flag){
					response.getWriter().println("<head></head><body onload=\"javascript:parent.location='/CDUG/jsp/backRelease/newsbackRelease.jsp';alert('新闻发布成功！');\"></body>");	
				}
				else{
				response.getWriter().println("<head></head><body onload=\"javascript:parent.location='/CDUG/jsp/backRelease/newsbackRelease.jsp';alert('新闻发布失败！');\"></body>");
				}
				dbctr.close();
			}
		}
